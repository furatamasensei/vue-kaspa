...
4. **COOP/COEP headers** — `Cross-Origin-Embedder-Policy: credentialless` and `Cross-Origin-Opener-Policy: same-origin` are set on the Vite dev server and via Nitro route rules for production. These are required for `SharedArrayBuffer`, which `kaspa-wasm` uses internally.
